...
namespace PS.Glb.New.Cadastros.Globais
{
    public partial class frmStatus : Form
    {
            ...
            if (edita == false)
            {
                try
                {
                    if (!string.IsNullOrEmpty(txtCodStatus.Text) && !string.IsNullOrEmpty(cmbTabela.Text.ToString()))
                    {
                        ...
                        conn.Open();
                        System.Data.SqlClient.SqlCommand command = new System.Data.SqlClient.SqlCommand("INSERT INTO GSTATUS (CODEMPRESA, CODSTATUS, DESCRICAO, IMAGEM, TABELA) VALUES (@CODEMPRESA, @CODSTATUS, @DESCRICAO, @IMAGEM, @TABELA)", conn);
                        ...
                        command.Parameters.Add(new System.Data.SqlClient.SqlParameter("@CODSTATUS", SqlDbType.VarChar)).Value = txtCodStatus.Text;
            ...
            else
            {
                try
                {
                    if (!string.IsNullOrEmpty(txtCodStatus.Text) && !string.IsNullOrEmpty(cmbTabela.Text.ToString()))
                    {
                        System.Data.SqlClient.SqlConnection conn = new System.Data.SqlClient.SqlConnection(AppLib.Context.poolConnection.Get("Start").ConnectionString);
                        conn.Open();
                        System.Data.SqlClient.SqlCommand command = new System.Data.SqlClient.SqlCommand("UPDATE GSTATUS SET  DESCRICAO = @DESCRICAO, IMAGEM = @IMAGEM, TABELA = @TABELA WHERE CODEMPRESA = @CODEMPRESA AND CODSTATUS = @CODSTATUS AND TABELA = @TABELA", conn);
                        command.Parameters.Add(new System.Data.SqlClient.SqlParameter("@CODEMPRESA", SqlDbType.Int)).Value = Convert.ToInt32(txtCodEmpresa.Text);
                        command.Parameters.Add(new System.Data.SqlClient.SqlParameter("@CODSTATUS", SqlDbType.VarChar)).Value = txtCodStatus.Text;
                        command.Parameters.Add(new System.Data.SqlClient.SqlParameter("@DESCRICAO", SqlDbType.VarChar)).Value = txtDescricao.Text;
                        command.Parameters.Add(new System.Data.SqlClient.SqlParameter("@IMAGEM", SqlDbType.Image)).Value = ConverterFotoParaByteArray();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            object bArray = (object)AppLib.Context.poolConnection.Get("Start").ExecGetField(string.Empty, "SELECT IMAGEM FROM GSTATUS", new object[] { });
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GSTATUS WHERE CODEMPRESA = ? AND CODSTATUS = ? AND TABELA = ?", new object[] { codEmpresa, CodStatus, Tabela });
            if (dt.Rows.Count > 0)
            {
                txtCodEmpresa.Text = dt.Rows[0]["CODEMPRESA"].ToString();
                txtCodStatus.Text = dt.Rows[0]["CODSTATUS"].ToString();
        ...
        private void peImagem_EditValueChanged(object sender, EventArgs e)
        {
