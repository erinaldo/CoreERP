...
{
    public partial class frmAprovaLimiteCredito : Form
    {
        public List<string> ListaCodOper = new List<string>();

        public frmAprovaLimiteCredito()
        {
            InitializeComponent();
        }
        ...
        {
            PS.Glb.New.Visao.Globais.frmVisaoMotivo frm = new PS.Glb.New.Visao.Globais.frmVisaoMotivo(" WHERE GMOTIVOUTILIZACAO.UTILIZACAO = 'Aprovação de Limite de Crédito' ");
            frm.consulta = true;
        ...
        private void btnExecutar_Click(object sender, EventArgs e)
        {
            ...
            {
                AppLib.Context.poolConnection.Get("Start").ExecTransaction(@"INSERT INTO GOPERAPROV (CODEMPRESA, CODOPER, USUARIO, DATA, CODMOTIVO, OBS, TIPO) VALUES (?,?,?,?,?, ?, ?)", new object[] { AppLib.Context.Empresa, ListaCodOper[i], AppLib.Context.Usuario, AppLib.Context.poolConnection.Get("Start").GetDateTime(), txtCodMotivo.Text, txtObservacao.Text, "LIMITE CREDITO" });

                AppLib.Context.poolConnection.Get("Start").ExecTransaction(@"UPDATE GOPER SET CODSITUACAO = NULL WHERE CODOPER = ? AND CODEMPRESA = ?", new object[] { ListaCodOper[i], AppLib.Context.Empresa });
            }

            MessageBox.Show("Operação(ões) selecionada(s) desbloqueada(s) com sucesso.", "Informação do Sistema.", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.Dispose();
        }
