...
{
    public partial class frmReprovaOperacao : Form
    {
        public List<string> ListaCodOper = new List<string>();

        public frmReprovaOperacao()
        {
            InitializeComponent();
        }
        ...
        {
            PS.Glb.New.Visao.Globais.frmVisaoMotivo frm = new PS.Glb.New.Visao.Globais.frmVisaoMotivo(" WHERE GMOTIVOUTILIZACAO.UTILIZACAO = 'Reprova Operação' ");
            frm.consulta = true;
        ...
        private void btnExecutar_Click(object sender, EventArgs e)
        {
            ...
            {
                AppLib.Context.poolConnection.Get("Start").ExecTransaction(@"INSERT INTO GOPERAPROV (CODEMPRESA, CODOPER, USUARIO, DATA, CODMOTIVO, OBS, TIPO) VALUES (?,?,?,?,?, ?, ?)", new object[] { AppLib.Context.Empresa, ListaCodOper[i], AppLib.Context.Usuario, AppLib.Context.poolConnection.Get("Start").GetDateTime(), txtCodMotivo.Text, txtObservacao.Text, "REPROVAÇÃO DA OPERAÇÃO" });
                AppLib.Context.poolConnection.Get("Start").ExecTransaction(@"UPDATE GOPER SET APROVACAO = '0' WHERE CODOPER = ? AND CODEMPRESA = ?", new object[] { ListaCodOper[i], AppLib.Context.Empresa });
            }
