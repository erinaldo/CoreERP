...
{
    public partial class frmSelecaoOpCopiaReferencia : Form
    {
        string tabela = "GOPER";
        public string codTipOper = string.Empty;
        public int codFilial = 0;

        public frmSelecaoOpCopiaReferencia()
        {
            InitializeComponent();
        }
...
LEFT OUTER JOIN GCIDADE ON GCIDADE.CODETD = VCLIFOR.CODETD AND GCIDADE.CODCIDADE = VCLIFOR.CODCIDADE";
        ...
        {
            List<string> listCodOper = new List<string>();
        
            for (int i = 0; i < gridView1.SelectedRowsCount; i++)
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(i).ToString()));
                listCodOper.Add(row1["GOPER.CODOPER"].ToString());
            }

            List<Class.GoperItem> listaItens = new Class.GoperItem().getGoperItemFaturamento(listCodOper, AppLib.Context.poolConnection.Get("Start"));
            Visao.frmVisaoItensCopiaReferencia frm = new Visao.frmVisaoItensCopiaReferencia(listaItens);
            frm.codFilial = codFilial;
            frm.codTipOperDestino = codTipOper;
            frm.ShowDialog();
            this.Dispose();
        }
        ...
        private void frmSelecaoOpCopiaReferencia_Load(object sender, EventArgs e)
        {
