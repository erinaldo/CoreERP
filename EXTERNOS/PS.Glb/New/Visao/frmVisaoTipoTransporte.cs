...
{
    public partial class frmVisaoTipoTransporte : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VTIPOTRANSPORTE";
        ...
        public frmVisaoTipoTransporte(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTipoTransporte(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTipoTransporte_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnCadastro_TipoTransporte", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTIPOTRANSPORTE WHERE CODTIPOTRANSPORTE = ? AND CODEMPRESA = ?", new object[] { dr["VTIPOTRANSPORTE.CODTIPOTRANSPORTE"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroTipoTransporte tipoTransp = new Cadastros.frmCadastroTipoTransporte();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    tipoTransp.CodTipoTransporte = row1["VTIPOTRANSPORTE.CODTIPOTRANSPORTE"].ToString();
                    tipoTransp.edita = true;
                    tipoTransp.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODTIPOTRANSPORTE"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODTIPOTRANSPORTE"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroTipoTransporte tipoTransp = new Cadastros.frmCadastroTipoTransporte();
                tipoTransp.edita = false;
                tipoTransp.ShowDialog();
                CarregaGrid(query);
            }
            else
            {
                PS.Glb.New.Cadastros.frmCadastroTipoTransporte tipoTransp = new Cadastros.frmCadastroTipoTransporte(ref this.lookup);
                tipoTransp.edita = false;
                tipoTransp.ShowDialog();
                this.Dispose();
            }
