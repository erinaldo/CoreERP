...
{
    public partial class frmVisaoTipoRua : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GTIPORUA";
        ...
        public string CodTipoRua;
        public string Nome;

        //Variaveis para NewLookup
        private NewLookup lookup;
        public frmVisaoTipoRua(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTipoRua(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoTipoRua(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTipoRua_Load(object sender, EventArgs e)
        {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GTIPORUA WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GTIPORUA WHERE CODTIPORUA = ?", new object[] { dr["GTIPORUA.CODTIPORUA"] });
        ...
        private void Atualizar()
        {
            ...
            lookup.txtcodigo.Text = row1[tabela + ".CODTIPORUA"].ToString();
            lookup.ValorCodigoInterno = row1[tabela + ".CODTIPORUA"].ToString();
            this.Dispose();
        }
