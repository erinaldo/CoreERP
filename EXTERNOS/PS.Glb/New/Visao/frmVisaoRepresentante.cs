...
{
    public partial class frmVisaoRepresentante : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VREPRE";
        ...
        public frmVisaoRepresentante(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoRepresentante(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoRepresentante_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnCadastro_Representante", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREPRE WHERE CODREPRE = ? AND CODEMPRESA = ?", new object[] { dr["VREPRE.CODREPRE"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroRepresentante Repre = new Cadastros.frmCadastroRepresentante();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Repre.CodRepre = row1["VREPRE.CODREPRE"].ToString();
                    Repre.edita = true;
                    Repre.codMenu = codMenu;
                    Repre.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODREPRE"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODREPRE"].ToString();
                this.Dispose();
            }
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodRepresentante = row1["VREPRE.CODREPRE"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODREPRE", "VREPRE", row["VREPRE.CODREPRE"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
