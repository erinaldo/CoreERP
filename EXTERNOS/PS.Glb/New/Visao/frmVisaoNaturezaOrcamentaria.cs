...
{
    public partial class frmVisaoNaturezaOrcamentaria : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VNATUREZAORCAMENTO";
        ...
        public frmVisaoNaturezaOrcamentaria(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoNaturezaOrcamentaria(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoNaturezaOrcamentaria(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoNaturezaOrcamentaria_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosGlobais_NaturezaOrcamentaria", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT CODEMPRESA, CODNATUREZA, DESCRICAO, ATIVO, PERMITELANCAMENTO, NATUREZA FROM VNATUREZAORCAMENTO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT CODEMPRESA, CODNATUREZA, DESCRICAO, ATIVO, PERMITELANCAMENTO, NATUREZA FROM VNATUREZAORCAMENTO WHERE CODNATUREZA = ? AND CODEMPRESA = ?", new object[] { dr["VNATUREZAORCAMENTO.CODNATUREZA"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroNaturezaOrcamentaria natureza = new Cadastros.frmCadastroNaturezaOrcamentaria();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    natureza.CodNatureza = row1["VNATUREZAORCAMENTO.CODNATUREZA"].ToString();
                    natureza.edita = true;
                    natureza.codMenu = codMenu;
                    natureza.ShowDialog();
                    atualizaColuna(row1);
                }
            }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroNaturezaOrcamentaria natureza = new Cadastros.frmCadastroNaturezaOrcamentaria(ref this.lookup);
                natureza.edita = false;
                natureza.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodNatureza = row1["VNATUREZAORCAMENTO.CODNATUREZA"].ToString();
                    Descricao = row1["VNATUREZA.DESCRICAO"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODNATUREZA", "VNATUREZAORCAMENTO", row["VNATUREZAORCAMENTO.CODNATUREZA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
