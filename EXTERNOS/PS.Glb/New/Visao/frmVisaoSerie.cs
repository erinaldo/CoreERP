...
{
    public partial class frmVisaoSerie : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VSERIE";
        ...
        public frmVisaoSerie(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoSerie(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoSerie(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoSerie_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VSERIE WHERE CODSERIE = ? AND CODEMPRESA = ? AND CODFILIAL = ?", new object[] { dr["VSERIE.CODSERIE"], AppLib.Context.Empresa, AppLib.Context.Filial });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    serie.CodSerie = row1["VSERIE.CODSERIE"].ToString();
                     serie.Codfilial = row1["VSERIE.CODFILIAL"].ToString();
                    serie.edita = true;
                    serie.codMenu = codMenu;
                    serie.ShowDialog();
                    atualizaColuna(row1);
                }
            }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroSerie serie = new Cadastros.frmCadastroSerie(ref this.lookup);
                serie.edita = false;
                serie.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodSerie = row1["VSERIE.CODSERIE"].ToString();
                    Descricao = row1["VSERIE.CODSERIE"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODSERIE", "VSERIE", row["VSERIE.CODSERIE"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
