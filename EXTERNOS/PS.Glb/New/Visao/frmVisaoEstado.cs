...
{
    public partial class frmVisaoEstado : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GESTADO";
        ...
        public frmVisaoEstado(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoEstado(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoEstado(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoEstado_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosGlobais_Estado", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GESTADO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GESTADO WHERE CODETD = ?", new object[] { dr["GESTADO.CODETD"] });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroEstado estado = new Cadastros.frmCadastroEstado();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    estado.CodEstado = row1["GESTADO.CODETD"].ToString();
                    estado.edita = true;
                    estado.codMenu = codMenu;
                    estado.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODETD"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODETD"].ToString();
                this.Dispose();
            }
        }
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroEstado estado = new Cadastros.frmCadastroEstado(ref this.lookup);
                estado.edita = false;
                estado.ShowDialog();
                this.Dispose();
            }
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODETD", "GESTADO", row["GESTADO.CODETD"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodEstado = row1["GESTADO.CODETD"].ToString();
                    Nome = row1["GESTADO.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
