...
{
    public partial class frmVisaoFormaPagamento : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VFORMAPGTO";
        ...
        public frmVisaoFormaPagamento(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoFormaPagamento(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoFormaPagamento(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoFormaPagamento_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastro_Serie", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VFORMAPGTO WHERE CODFORMA = ? AND CODEMPRESA = ?", new object[] { dr["VFORMAPGTO.CODFORMA"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroFormaPagamento pagamento = new Cadastros.frmCadastroFormaPagamento();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    pagamento.CodForma = row1["VFORMAPGTO.CODFORMA"].ToString();
                    pagamento.edita = true;
                    pagamento.codMenu = codMenu;
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODFORMA"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODFORMA"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroFormaPagamento pagamento = new Cadastros.frmCadastroFormaPagamento();
                pagamento.edita = false;
                pagamento.ShowDialog();
                CarregaGrid(query, consulta);
            }
            else
            {
                PS.Glb.New.Cadastros.frmCadastroFormaPagamento pagamento = new Cadastros.frmCadastroFormaPagamento(ref this.lookup);
                pagamento.edita = false;
                pagamento.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Codforma = row1["VFORMAPGTO.CODFORMA"].ToString();
                    Nome = row1["VFORMAPGTO.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODFORMA", "VFORMAPGTO", row["VFORMAPGTO.CODFORMA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
