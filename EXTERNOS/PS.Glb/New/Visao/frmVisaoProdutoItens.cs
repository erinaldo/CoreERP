...
{
    public partial class frmVisaoProdutoItens : Form
    {
        ...
        public frmVisaoProdutoItens(string _query, Form frmprin, string _Codmenu)
        {
            InitializeComponent();
        ...
        public frmVisaoProdutoItens(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Add("VSALDOESTOQUE");
            query = _where;
        ...
        public frmVisaoProdutoItens(ref NewLookup lookup)
        {
            InitializeComponent();
            ...
            /// João Pedro Luchiari - 05/12/2017
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnCadastrosGlobais_Produtos", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            codlocal = AppLib.Context.poolConnection.Get("Start").ExecGetField(string.Empty, "SELECT CODLOCAL FROM VSALDOESTOQUE", new object[] { }).ToString();
            relacionamento = "LEFT OUTER JOIN VSALDOESTOQUE ON VSALDOESTOQUE.CODEMPRESA = VPRODUTO.CODEMPRESA AND VSALDOESTOQUE.CODPRODUTO = VPRODUTO.CODPRODUTO AND VSALDOESTOQUE.CODFILIAL = '" + AppLib.Context.Filial + "' AND VSALDOESTOQUE.CODLOCAL = '" + codlocal + "'";
            ...
            try
            {
                ...
                sql = new Class.Utilidades().getVisao("GOPERITEMPRODUTOS", relacionamento, tabelasFilhas, where);

                sql = sql.Replace("GOPERITEMPRODUTOS", "VPRODUTO");
                ...
                DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT COLUNA, LARGURA FROM GVISAOUSUARIO WHERE VISAO = ? AND CODUSUARIO = ? AND VISIVEL = 1 ORDER BY SEQUENCIA", new object[] { "GOPERITEMPRODUTOS", AppLib.Context.Usuario });
        ...
        {
            frmSelecaoColunas frm = new frmSelecaoColunas("GOPERITEMPRODUTOS");
            frm.ShowDialog();
            carregaGrid(query, false);
        }
