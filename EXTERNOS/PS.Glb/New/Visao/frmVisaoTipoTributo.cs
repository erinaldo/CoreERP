...
{
    public partial class frmVisaoTipoTributo : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VTIPOTRIBUTO";
        ...
        public string CodTipoTributo;
        public string Descricao;

        //Variaveis para NewLookup
        private NewLookup lookup;
        public frmVisaoTipoTributo(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTipoTributo(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoTipoTributo(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTipoTributo_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTIPOTRIBUTO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTIPOTRIBUTO WHERE CODTIPOTRIBUTO = ? AND CODEMPRESA = ?", new object[] { dr["VTIPOTRIBUTO.CODTIPOTRIBUTO"], AppLib.Context.Empresa });
        ...
        { 
            if (this.lookup == null)
            {
                New.Cadastros.frmCadastroTipoTributo tipo = new Cadastros.frmCadastroTipoTributo();
                tipo.edita = false;
                tipo.ShowDialog();
            ...
            {
                New.Cadastros.frmCadastroTipoTributo tipo = new Cadastros.frmCadastroTipoTributo(ref this.lookup);
                tipo.edita = false;
                tipo.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    tipo.CodTipoTributo = row1["VTIPOTRIBUTO.CODTIPOTRIBUTO"].ToString();
                    tipo.edita = true;
                    tipo.codMenu = codMenu;
                    tipo.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODTIPOTRIBUTO"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODTIPOTRIBUTO"].ToString();
                this.Dispose();
            }
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodTipoTributo = row1["VTIPOTRIBUTO.CODTIPOTRIBUTO"].ToString();
                    Descricao = row1["GVTIPOTRIBUTO.CODTIPOTRIBUTO"].ToString();
                    this.Dispose();
                }
                else
                {
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
...

            {
                ...
                for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                {
                    ...
                    AppLib.Context.poolConnection.Get("Start").ExecTransaction("DELETE FROM VTIPOTRIBUTO WHERE CODTIPOTRIBUTO = ? AND CODEMPRESA = ?", new object[] { row["VTIPOTRIBUTO.CODTIPOTRIBUTO"], AppLib.Context.Empresa });
                }
