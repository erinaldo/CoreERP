...
{
    public partial class frmVisaoFabricante : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VFABRICANTE";
        ...
        public frmVisaoFabricante(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoFabricante(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoFabricante_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastro_Fabricante", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VFABRICANTE WHERE CODFABRICANTE = ? AND CODEMPRESA = ?", new object[] { dr["VFABRICANTE.CODFABRICANTE"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroFabricante fabricante = new Cadastros.frmCadastroFabricante();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    fabricante.CodFabricante = row1["VFABRICANTE.CODFABRICANTE"].ToString();
                    fabricante.edita = true;
                    fabricante.codMenu = codMenu;
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODFABRICANTE"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODFABRICANTE"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroFabricante fabricante = new Cadastros.frmCadastroFabricante();
                fabricante.edita = false;
                fabricante.ShowDialog();
                CarregaGrid(query);
            }
            else
            {
                PS.Glb.New.Cadastros.frmCadastroFabricante fabricante = new Cadastros.frmCadastroFabricante(ref this.lookup);
                fabricante.edita = false;
                fabricante.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodFabricante = row1["VFABRICANTE.CODFABRICANTE"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODFABRICANTE", "VFABRICANTE", row["VFABRICANTE.CODFABRICANTE"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
