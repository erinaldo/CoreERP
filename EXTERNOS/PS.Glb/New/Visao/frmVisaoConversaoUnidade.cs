...
{
    public partial class frmVisaoConversaoUnidade : Form
    {
        ...
        public frmVisaoConversaoUnidade(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
        ...
        private void frmVisaoConversaoUnidade_Load(object sender, EventArgs e)
        {
        ...
        public frmVisaoConversaoUnidade(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastro_FatorConversao", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODUNID", "VUNIDCONVERSAO", row["VUNIDCONVERSAO.CODUNID"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void excelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                reportPath = open.SelectedPath + "FatorConversao.xlsx";
                ...
                xlsxOptions.SheetName = "FatorConversao";
