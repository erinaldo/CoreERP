...
namespace PS.Glb.New.Visao
{
    public partial class frmVisaoCentroCusto : Form
    {
        ...
        public string CodCusto;
        public string Nome;

        //Variaveis para NewLookup
        private NewLookup lookup;
        public frmVisaoCentroCusto(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoCentroCusto(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosGlobais_CentrodeCusto", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GCENTROCUSTO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GCENTROCUSTO WHERE CODCCUSTO = ? AND CODEMPRESA = ?", new object[] { dr["GCENTROCUSTO.CODCCUSTO"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroCentroCusto custo = new Cadastros.frmCadastroCentroCusto();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    custo.CodCusto = row1["GCENTROCUSTO.CODCCUSTO"].ToString();
                    custo.edita = true;
                    custo.codMenu = codMenu;
                    custo.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODCCUSTO"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODCCUSTO"].ToString();
                this.Dispose();
            }
        }
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroCentroCusto custo = new Cadastros.frmCadastroCentroCusto(ref this.lookup);
                custo.edita = false;
                custo.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodCusto = row1["GCENTROCUSTO.CODCCUSTO"].ToString();
                    Nome = row1["GCENTROCUSTO.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODCCUSTO", "GCENTROCUSTO", row["GCENTROCUSTO.CODCCUSTO"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void frmVisaoCentroCusto_Load(object sender, EventArgs e)
        {
