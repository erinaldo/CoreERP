...
{
    public partial class frmVisaoRegraIcms : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VREGRAICMS";
        ...
        public frmVisaoRegraIcms(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoRegraIcms(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoRegraIcms(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosFiscais_RegrasIcms", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREGRAICMS WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREGRAICMS WHERE IDREGRA = ? AND CODEMPRESA = ?", new object[] { dr["VREGRAICMS.IDREGRA"], AppLib.Context.Empresa });
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                New.Cadastros.frmCadastroRegraIcms icms = new Cadastros.frmCadastroRegraIcms();
                icms.edita = false;
                icms.ShowDialog();
                CarregaGrid(query, false);
            }
            ...
            {
                New.Cadastros.frmCadastroRegraIcms icms = new Cadastros.frmCadastroRegraIcms(ref this.lookup);
                icms.edita = false;
                icms.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    icms.IdRegra = row1["VREGRAICMS.IDREGRA"].ToString();
                    icms.edita = true;
                    icms.codMenu = codMenu;
                    icms.ShowDialog();
                    atualizaColuna(row1);
                }
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    IdRegra = row1["VREGRAICMS.IDREGRA"].ToString();
                    Descricao = row1["VREGRAICMS.IDREGRA"].ToString();
                    this.Dispose();
                }
                else
                {
