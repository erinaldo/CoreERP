...
{
    public partial class frmVisaoBanco : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GBANCO";
        ...
        public frmVisaoBanco(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoBanco(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoBanco_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnMovimentacoesBancarias_Bancos", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GBANCO WHERE CODBANCO = ? AND CODEMPRESA = ?", new object[] { dr["GBANCO.CODBANCO"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroBanco Banco = new Cadastros.frmCadastroBanco();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Banco.CodBanco = row1["GBANCO.CODBANCO"].ToString();
                    Banco.edita = true;
                    Banco.codMenu = codMenu;
                    Banco.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODBANCO"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODBANCO"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroBanco Banco = new Cadastros.frmCadastroBanco(ref this.lookup);
                Banco.edita = false;
                Banco.ShowDialog();
                this.Dispose();
            }
        ...
        {
            if (MessageBox.Show("Deseja excluir o(s) registro(s) selecionado(s)?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        AppLib.Context.poolConnection.Get("Start").ExecTransaction("DELETE FROM GBANCO WHERE CODBANCO = ? AND CODEMPRESA = ?", new object[] { row["GBANCO.CODBANCO"], AppLib.Context.Empresa });
                    }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodBanco = row1["GBANCO.CODBANCO"].ToString();
                    this.Dispose();
                }
                else
                {
