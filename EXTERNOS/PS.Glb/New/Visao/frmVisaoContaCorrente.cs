...
{
    public partial class frmVisaoContaCorrente : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "FCCORRENTE";
        ...
        public frmVisaoContaCorrente(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoContaCorrente(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoContaCorrente_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM FCCORRENTE WHERE NUMCONTA = ? AND CODEMPRESA = ?", new object[] { dr["FCCORRENTE.NUMCONTA"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroContaCorrente Conta = new Cadastros.frmCadastroContaCorrente();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Conta.CodConta = row1["FCCORRENTE.NUMCONTA"].ToString();
                    Conta.edita = true;
                    Conta.codMenu = codMenu;
                    Conta.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            ...
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                lookup.txtconteudo.Text = row1[tabela + ".DIGCONTA"].ToString().ToUpper();
                lookup.txtcodigo.Text = row1[tabela + ".NUMCONTA"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".NUMCONTA"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroContaCorrente Conta = new Cadastros.frmCadastroContaCorrente(ref this.lookup);
                Conta.edita = false;
                Conta.ShowDialog();
                this.Dispose();
            }
            ...
            if (MessageBox.Show("Deseja excluir o(s) registro(s) selecionados(?)", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("NUMCONTA", "FCCORRENTE", row["FCCORRENTE.NUMCONTA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodConta = row1["FCCORRENTE.NUMCONTA"].ToString();
                    this.Dispose();
                }
                else
                {
