...
{
    public partial class frmVisaoMensagensPadrao : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VOPERMENSAGEM";
        ...
        public frmVisaoMensagensPadrao(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoMensagensPadrao(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoMensagensPadrao_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastro_MensagensOperacao", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                //{
                //    dic = new DataTable();
                //    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VOPERMENSAGEM WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                //    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VOPERMENSAGEM WHERE CODMENSAGEM = ? AND CODEMPRESA = ?", new object[] { dr["VOPERMENSAGEM.CODMENSAGEM"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroMensagensPadrao msg = new Cadastros.frmCadastroMensagensPadrao();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    msg.CodMensagem = row1["VOPERMENSAGEM.CODMENSAGEM"].ToString();
                    msg.edita = true;
                    msg.codMenu = codMenu;
                    msg.ShowDialog();
                    atualizaColuna(row1);
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODMENSAGEM"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODMENSAGEM"].ToString();
                this.Dispose();
            }
        }
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroMensagensPadrao msg = new Cadastros.frmCadastroMensagensPadrao(ref this.lookup);
                msg.edita = false;
                msg.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodMensagem = row1["VOPERMENSAGEM.CODMENSAGEM"].ToString();
                    Mensagem = row1["VOPERMENSAGEM.MENSAGEM"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODMENSAGEM", "VOPERMENSAGEM", row["VOPERMENSAGEM.CODMENSAGEM"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
