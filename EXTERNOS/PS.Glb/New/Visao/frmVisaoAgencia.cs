...
{
    public partial class frmVisaoAgencia : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GAGENCIA";
        ...
        public frmVisaoAgencia(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoAgencia(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoAgencia_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GAGENCIA WHERE CODAGENCIA = ? AND CODEMPRESA = ?", new object[] { dr["GAGENCIA.CODAGENCIA"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroAgencia Agencia = new Cadastros.frmCadastroAgencia();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Agencia.CodAgencia = row1["GAGENCIA.CODAGENCIA"].ToString();
                    Agencia.edita = true;
                    Agencia.codMenu = codMenu;
                    Agencia.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODAGENCIA"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODAGENCIA"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroAgencia Agencia = new Cadastros.frmCadastroAgencia(ref this.lookup);
                Agencia.edita = false;
                Agencia.ShowDialog();
                this.Dispose();
            }
            ...
            if (MessageBox.Show("Deseja excluir o(s) registro(s) selecionados(?)", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODAGENCIA", "GAGENCIA", row["GAGENCIA.CODAGENCIA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodAgencia = row1["GAGENCIA.CODAGENCIA"].ToString();
                    this.Dispose();
                }
                else
                {
