...
{
    public partial class frmVisaoTributos : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VTRIBUTO";
        ...
        public frmVisaoTributos(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTributos(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoTributos(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTributos_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosFiscais_Regiao", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTRIBUTO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTRIBUTO WHERE CODTRIBUTO = ? AND CODEMPRESA = ?", new object[] { dr["VTRIBUTO.CODTRIBUTO"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroTributos tributo = new Cadastros.frmCadastroTributos();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    tributo.CodTributo = row1["VTRIBUTO.CODTRIBUTO"].ToString();
                    tributo.edita = true;
                    tributo.ShowDialog();
                    atualizaColuna(row1);
                }
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODTRIBUTO"].ToString();  //////
                lookup.ValorCodigoInterno = row1[tabela + ".CODTRIBUTO"].ToString(); /////
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroTributos tributo = new Cadastros.frmCadastroTributos(ref this.lookup);
                tributo.edita = false;
                tributo.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodTributo = row1["VTRIBUTO.CODTRIBUTO"].ToString();
                    Descricao = row1["VTRIBUTO.DESCRICAO"].ToString();
                    this.Dispose();
                }
                else
                {
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
...

            {
                ...
                for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                {
                    ...
                    AppLib.Context.poolConnection.Get("Start").ExecTransaction("DELETE FROM VTRIBUTO WHERE CODTRIBUTO = ? AND CODEMPRESA = ?", new object[] { row["VTRIBUTO.CODTRIBUTO"], AppLib.Context.Empresa });
                }
