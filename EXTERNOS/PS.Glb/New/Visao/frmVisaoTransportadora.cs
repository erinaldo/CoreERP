...
{
    public partial class frmVisaoTransportadora : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VTRANSPORTADORA";
        ...
        public frmVisaoTransportadora(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTransportadora(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTransportadora_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnCadastro_Transportadora", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VTRANSPORTADORA WHERE CODTRANSPORTADORA = ? AND CODEMPRESA = ?", new object[] { dr["VTRANSPORTADORA.CODTRANSPORTADORA"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroTransportadora transp = new Cadastros.frmCadastroTransportadora();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    transp.Codtransportadora = row1["VTRANSPORTADORA.CODTRANSPORTADORA"].ToString();
                    transp.edita = true;
                    transp.ShowDialog();
                    atualizaColuna(row1);
                }
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODTRANSPORTADORA"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODTRANSPORTADORA"].ToString();
                this.Dispose();
            }
        }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroTransportadora transp = new Cadastros.frmCadastroTransportadora(ref this.lookup);
                transp.edita = false;
                transp.ShowDialog();
                this.Dispose();
            }
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODTRANSPORTADORA", "VTRANSPORTADORA", row["VTRANSPORTADORA.CODTRANSPORTADORA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
