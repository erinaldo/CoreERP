...
{
    public partial class frmVisaoCheque : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "FCHEQUE";
        ...
        public frmVisaoCheque(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoCheque(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoCheque_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnLancamentos_Cheques", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VSERIE WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            Filtro.frmFiltroCheque Cheque = new Filtro.frmFiltroCheque();
            Cheque.aberto = true;
            Cheque.ShowDialog();
            if (!string.IsNullOrEmpty(Cheque.condicao))
            {
                query = Cheque.condicao;
                CarregaGrid(query);
            }
        }
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM FCHEQUE WHERE CODCHEQUE = ? AND CODEMPRESA = ?", new object[] { dr["FCHEQUE.CODCHEQUE"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroCheque Cheque = new Cadastros.frmCadastroCheque(false, null);
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    Cheque.Codcheque = row1["FCHEQUE.CODCHEQUE"].ToString();
                    Cheque.edita = true;
                    Cheque.ShowDialog();
                    atualizaColuna(row1);
                }
            ...
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                lookup.txtconteudo.Text = row1[tabela + ".CODCHEQUE"].ToString().ToUpper();
                lookup.txtcodigo.Text = row1[tabela + ".CODCHEQUE"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODCHEQUE"].ToString();
                this.Dispose();
            }
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodCheque = row1["FFCHEQUE.CODCHEQUE"].ToString();
                    this.Dispose();
                }
                else
                {
        ...
        private void imprimirCópiaDeChequeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Deseja imprimir a cópia de cheque?", "Informação do Sistema.", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(i).ToString()));
                    PS.Lib.WinForms.Report.ReportDesignTool rp = new PS.Lib.WinForms.Report.ReportDesignTool(new Relatorios.XrCheque(row1["FCHEQUE.CODCHEQUE"].ToString(), AppLib.Context.Empresa));
