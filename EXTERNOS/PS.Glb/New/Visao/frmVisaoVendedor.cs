...
{
    public partial class frmVisaoVendedor : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VVENDEDOR";
        ...
        public frmVisaoVendedor(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoVendedor(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoVendedor_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODUSUARIO = ?", new object[] { "btnCadastro_Vendedor", AppLib.Context.Usuario });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VVENDEDOR WHERE CODVENDEDOR = ? AND CODEMPRESA = ?", new object[] { dr["VVENDEDOR.CODVENDEDOR"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    PS.Glb.New.Cadastros.frmCadastroVendedor vendedor = new Cadastros.frmCadastroVendedor();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    vendedor.CodVendedor = row1["VVENDEDOR.CODVENDEDOR"].ToString();
                    vendedor.edita = true;
                    vendedor.codMenu = codMenu;
                    vendedor.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODVENDEDOR"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODVENDEDOR"].ToString();
                this.Dispose();
            }
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodVendedor = row1["VVENDEDOR.CODVENDEDOR"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODVENDEDOR", "VVENDEDOR", row["VVENDEDOR.CODVENDEDOR"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
