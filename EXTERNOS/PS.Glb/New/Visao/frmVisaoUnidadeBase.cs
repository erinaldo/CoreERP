...
{
    public partial class frmVisaoUnidadeBase : Form
    {
        ...
        public frmVisaoUnidadeBase(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoUnidadeBase(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoUnidadeBase(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoUnidadeBase_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastro_UnidadeMedida", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VUNID WHERE CODUNID = ? AND CODEMPRESA = ?", new object[] { dr["VUNID.CODUNID"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroUnidadeMedida unidade = new Cadastros.frmCadastroUnidadeMedida();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    unidade.CodUnidade = row1["VUNID.CODUNID"].ToString();
                    unidade.edita = true;
                    unidade.codMenu = codMenu;
                    unidade.ShowDialog();
                    atualizaColuna(row1);
                }
            }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroUnidadeMedida unidade = new Cadastros.frmCadastroUnidadeMedida(ref this.lookup);
                unidade.edita = false;
                unidade.ShowDialog();
                this.Dispose();
            }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    PS.Glb.New.Cadastros.frmCadastroUnidadeMedida unidade = new Cadastros.frmCadastroUnidadeMedida();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    unidade.CodUnidade = row1["VUNID.CODUNID"].ToString();
                    unidade.edita = true;
                    unidade.ShowDialog();
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodUnid = row1["VUNID.CODUNID"].ToString();
                    Nome = row1["VUNID.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODUNID", "VUNID", row["VUNID.CODUNID"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
