...
{
    public partial class frmVisaoRegraIpi : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VREGRAIPI";
        ...
        public string IdRegra;
        public string Descricao;

        //Variaveis para NewLookup
        private NewLookup lookup;
        public frmVisaoRegraIpi(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoRegraIpi(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoRegraIpi(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoRegraIpi_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosFiscais_RegrasIpi", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREGRAIPI WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREGRAIPI WHERE IDREGRA = ? AND CODEMPRESA = ?", new object[] { dr["VREGRAIPI.IDREGRA"], AppLib.Context.Empresa });
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                New.Cadastros.frmCadastroRegraIpi ipi = new Cadastros.frmCadastroRegraIpi();
                ipi.edita = false;
                ipi.ShowDialog();
                CarregaGrid(query, false);
            }
            ...
            {
                New.Cadastros.frmCadastroRegraIpi ipi = new Cadastros.frmCadastroRegraIpi(ref this.lookup);
                ipi.edita = false;
                ipi.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    ipi.IdRegra = row1["VREGRAIPI.IDREGRA"].ToString();
                    ipi.edita = true;
                    ipi.codMenu = codMenu;
                    ipi.ShowDialog();
                    atualizaColuna(row1);
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".IDREGRA"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".IDREGRA"].ToString();
                this.Dispose();
            }
        }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    IdRegra = row1["VREGRAIPI.IDREGRA"].ToString();
                    Descricao = row1["VREGRAIPI.IDREGRA"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("IDREGRA", "VREGRAIPI", row["VREGRAIPI.IDREGRA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
