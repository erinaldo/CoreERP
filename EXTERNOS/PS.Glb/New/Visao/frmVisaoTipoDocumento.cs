...
{
    public partial class frmVisaoTipoDocumento : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "FTIPDOC";
        ...
        public frmVisaoTipoDocumento(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoTipoDocumento(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoTipoDocumento(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoTipoDocumento_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnMovimentacoesBancariass_TipoDocumento", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM FTIPDOC WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM FTIPDOC WHERE CODTIPDOC = ? AND CODEMPRESA = ?", new object[] { dr["FTIPDOC.CODTIPDOC"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroTipoDocumento doc = new Cadastros.frmCadastroTipoDocumento();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    doc.CodTipDoc = row1["FTIPDOC.CODTIPDOC"].ToString();
                    doc.edita = true;
                    doc.codMenu = codMenu;
                    doc.ShowDialog();
                    atualizaColuna(row1);
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODTIPDOC"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODTIPDOC"].ToString();
                this.Dispose();
            }
        }
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroTipoDocumento doc = new Cadastros.frmCadastroTipoDocumento(ref this.lookup);
                doc.edita = false;
                doc.ShowDialog();
                this.Dispose();
            }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodTipDoc = row1["FTIPODOC.CODTIPDOC"].ToString();
                    Nome = row1["FTIPDOC.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODTIPDOC", "FTIPDOC", row["FTIPDOC.CODTIPDOC"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
