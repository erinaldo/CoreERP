...
{
    public partial class frmVisaoRedeCartao : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VREDECARTAO";
        ...
        public string CodRedeCartao;
        public string Nome;

        //Variaveis para NewLookup
        private NewLookup lookup;

        public frmVisaoRedeCartao(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoRedeCartao(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoRedeCartao(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoRedeCartao_Load(object sender, EventArgs e)
        {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GTIPOBAIRRO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VREDECARTAO WHERE CODREDECARTAO = ?", new object[] { dr["VREDECARTAO.CODREDECARTAO"] });
        ...
        private void Atualizar()
        {
            ...
            lookup.txtcodigo.Text = row1[tabela + ".CODREDECARTAO"].ToString();
            lookup.ValorCodigoInterno = row1[tabela + ".CODREDECARTAO"].ToString();
            this.Dispose();
        }
