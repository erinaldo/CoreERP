...
{
    public partial class frmVisaoUsuarios : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GUSUARIO";
        string query = string.Empty;
        public string codMenu = string.Empty;
        private bool permiteEditar = true;
        PSPartUsuario user = new PSPartUsuario();
        List<string> UsuariosDestino = new List<string>();
        ...
        public frmVisaoUsuarios(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoUsuarios(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoUsuarios(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoUsuarios_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnSeguranca_Usuarios", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
            ...
            try
            {
                ...
                for (int i = 0; i < gridView1.Columns.Count; i++)
                {
                    ...
                    if (result != null)
                    {
                        ...
                        //Formata a grid para ter data e hora
                        if (gridView1.Columns[i].Caption == "Último Login")
                        {
                            gridView1.Columns[i].DisplayFormat.FormatString = "dd/MM/yyyy hh:mm:ss";
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GUSUARIO WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GUSUARIO WHERE CODUSUARIO = ?", new object[] { dr["GUSUARIO.CODUSUARIO"] });
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                New.Cadastros.frmCadastroUsuarios usuario = new Cadastros.frmCadastroUsuarios();
                usuario.edita = false;
                usuario.ShowDialog();
            ...
            {
                New.Cadastros.frmCadastroUsuarios usuario = new Cadastros.frmCadastroUsuarios(ref this.lookup);
                usuario.edita = false;
                usuario.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    usuario.CodUsuario = row1["GUSUARIO.CODUSUARIO"].ToString();
                    usuario.edita = true;
                    usuario.codMenu = codMenu;
                    usuario.ShowDialog();
                    atualizaColuna(row1);
                }
            }
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODUSUARIO"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODUSUARIO"].ToString();
                this.Dispose();
            }
        }
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODUSUARIO", "GUSUARIO", row["GUSUARIO.CODUSUARIO"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
        ...
        private void copiarPermissãoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gridView1.SelectedRowsCount == 0)
            {
            ...
            else
            {
                for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                {
                    ...
                    UsuariosDestino.Add(row["GUSUARIO.CODUSUARIO"].ToString());
                }

                PS.Glb.New.Processos.frmCopiaPermissaoUsuario frm = new Processos.frmCopiaPermissaoUsuario();
                frm.UsuarioDestino = UsuariosDestino;
                frm.ShowDialog();
            }          
        }

        private void sincronizarUsuáriosToolStripMenuItem_Click(object sender, EventArgs e)
        {
            PS.Glb.New.Processos.frmSincronizarUsuarios frm = new Processos.frmSincronizarUsuarios();
            frm.ShowDialog();
        }
    }
}
