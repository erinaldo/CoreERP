...
{
    public partial class frmVisaoOperador : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "VOPERADOR";
        ...
        public frmVisaoOperador(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoOperador(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoOperador(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoOperador_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosGlobais_Operador", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VOPERADOR WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VOPERADOR WHERE CODOPERADOR = ? AND CODEMPRESA = ?", new object[] { dr["VOPERADOR.CODOPERADOR"], AppLib.Context.Empresa });
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                {
                    New.Cadastros.frmCadastroOperador operador = new frmCadastroOperador();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    operador.CodOperador = row1["VOPERADOR.CODOPERADOR"].ToString();
                    operador.edita = true;
                    operador.ShowDialog();
                    atualizaColuna(row1);
                }
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                New.Cadastros.frmCadastroOperador operador = new frmCadastroOperador();
                operador.edita = false;
                operador.ShowDialog();
                CarregaGrid(query, consulta);
            }
            else
            {
                New.Cadastros.frmCadastroOperador operador = new frmCadastroOperador(ref this.lookup);
                operador.edita = false;
                operador.ShowDialog();
                this.Dispose();
            }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    New.Cadastros.frmCadastroOperador oper = new frmCadastroOperador();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    oper.CodOperador = row1["VOPERADOR.CODOPERADOR"].ToString();
                    oper.edita = true;
                    oper.ShowDialog();
                    atualizaColuna(row1);
                }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodOperador = row1["VOPERADOR.CODOPERADOR"].ToString();
                    Nome = row1["VOPERADOR.NOME"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODOPERADOR", "VOPERADOR", row["VOPERADOR.CODOPERADOR"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
