...
{
    public partial class frmVisaoNatureza : Form
    {
        string tabela = "VNATUREZA";
        ...
        public frmVisaoNatureza(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
        ...
        public frmVisaoNatureza(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoNatureza(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void frmVisaoNatureza_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosFiscais_NaturezaOperacao", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VNATUREZA WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            Filtro.frmFiltroNaturezaOperacao natureza = new Filtro.frmFiltroNaturezaOperacao();
            natureza.aberto = true;
            natureza.ShowDialog();
            if (!string.IsNullOrEmpty(natureza.condicao))
            {
                query = natureza.condicao;
                CarregaGrid(query, false);
            }
        }
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM VNATUREZA WHERE CODNATUREZA = ? AND CODEMPRESA = ?", new object[] { dr["VNATUREZA.CODNATUREZA"], AppLib.Context.Empresa });
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                PS.Glb.New.Cadastros.frmCadastroNatureza natureza = new Cadastros.frmCadastroNatureza();
            ...
            {
                PS.Glb.New.Cadastros.frmCadastroNatureza natureza = new Cadastros.frmCadastroNatureza(ref this.lookup);
                natureza.edita = false;
                natureza.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    ...
                    natureza.CodNatureza = row1["VNATUREZA.CODNATUREZA"].ToString();
                    natureza.edita = true;
                    natureza.ShowDialog();
                    atualizaColuna(row1);
                }
        ...
        private void gridControl1_DoubleClick(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                ...
                {
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    CodNatureza = row1["VNATUREZA.CODNATUREZA"].ToString();
                    Descricao = row1["VNATUREZA.CODNATUREZA"].ToString();
                    this.Dispose();
                }
                else
                {
            ...
            if (MessageBox.Show("Deseja excluir os registros selecionados?", "Confirmação.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    for (int i = 0; i < gridView1.SelectedRowsCount; i++)
                    {
                        ...
                        if (new Class.Utilidades().Excluir("CODNATUREZA", "VNATUREZA", row["VNATUREZA.CODNATUREZA"].ToString()) == true)
                        {
                            if (gridView1.SelectedRowsCount > 0)
                            {
                                continue;
