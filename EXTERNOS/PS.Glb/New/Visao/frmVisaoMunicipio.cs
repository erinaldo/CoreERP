...
{
    public partial class frmVisaoMunicipio : Form
    {
        public Form pai = null;
        public bool consulta = false;
        string tabela = "GCIDADE";
        ...
        public frmVisaoMunicipio(string _query, Form frmprin, string _CodMenu)
        {
            InitializeComponent();
            codMenu = _CodMenu;
        ...
        public frmVisaoMunicipio(string _where, bool _consulta)
        {
            InitializeComponent();
            tabelasFilhas.Clear();
            query = _where;
        ...
        public frmVisaoMunicipio(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT EDICAO, EXCLUSAO, CONSULTA, INCLUSAO FROM GPERMISSAOMENU WHERE CODMENU = ? AND CODPERFIL = ?", new object[] { "btnCadastrosGlobais_Municipio", AppLib.Context.Perfil });
            if (dt.Rows.Count > 0)
            {
        ...
        public void CarregaGrid(string where, bool consulta)
        {
            try
            {
                ...
                {
                    dic = new DataTable();
                    dic = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GCIDADE WHERE TABELA = ?", new object[] { tabelasFilhas[i].ToString() });
                    dt = new DataTable();
        ...
        {
            DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM GCIDADE WHERE CODCIDADE = ?", new object[] { dr["GCIDADE.CODCIDADE"] });
        ...
        private void Atualizar()
        {
            ...
            else
            {
                ...
                lookup.txtcodigo.Text = row1[tabela + ".CODCIDADE"].ToString();
                lookup.ValorCodigoInterno = row1[tabela + ".CODCIDADE"].ToString();
                this.Dispose();
            }
        }

        private void frmVisaoMunicipio_Load(object sender, EventArgs e)
        {
            btnNovo.Enabled = false;
            btnEditar.Enabled = false;
            btnExcluir.Enabled = false;
