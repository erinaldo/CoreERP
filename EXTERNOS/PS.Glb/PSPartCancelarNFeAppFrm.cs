...
namespace PS.Glb
{
    public partial class PSPartCancelarNFeAppFrm : PS.Lib.WinForms.FrmBaseApp
    {
        PS.Lib.Global gb = new PS.Lib.Global();

        public PSPartCancelarNFeAppFrm()
        {
            InitializeComponent();
        }
                ...
                if (psPartApp.DataGrid != null)
                {
                    for (int i = 0; i < psPartApp.DataGrid.Rows.Count; i++)
                    {
                        if (psPartApp.DataGrid.Rows[i].Selected)
                        {
                            ...
                            string boleto = AppLib.Context.poolConnection.Get("Start").ExecGetField(string.Empty, "SELECT BOLETOGERADO FROM FLANCA WHERE CODOPER = ? AND CODEMPRESA = ?", new object[] { this.psPartApp.DataGrid.Rows[i].Cells["CODOPER"].Value.ToString(), AppLib.Context.Empresa }).ToString();
                            if (!string.IsNullOrEmpty(boleto))
                            {
                                if (boleto == "SIM")
                                {
                                    throw new Exception("Existe boleto gerado para essa NF-e.\nNF-e não pode ser cancelada.");
                                }
                            }
                        }
                    }
        ...
        public override Boolean Execute()
        {
            if (PS.Lib.PSMessageBox.ShowQuestion("Confirma o cancelamento do(s) registro(s) selecionado(s) ?") == System.Windows.Forms.DialogResult.Yes)
            {
                try
                {
                    ...
                    if (this.psPartApp.Access == PS.Lib.AppAccess.View)
                    {
                        if (psPartApp.DataGrid != null)
                        {
                            for (int i = 0; i < psPartApp.DataGrid.Rows.Count; i++)
                            {
                                if (psPartApp.DataGrid.Rows[i].Selected)
                                {
                                    CancelarNFe(gb.RetornaDataFieldByDataGridViewRow(psPartApp.DataGrid.Rows[i]));
                                }
                            }
                        }
                    }
                    ...
                    {
                        CancelarNFe(this.psPartApp.DataField);
                    }
        ...
        private void CancelarNFe(List<DataField> objArr)
        {
            try
            {
                ...
                psPartNFEstadualData.CancelarNFe(Convert.ToInt32(dfCODEMPRESA.Valor), Convert.ToInt32(dfCODOPER.Valor), PS.Lib.Utils.RemoveCaracterSpecial(psMemoBox1.Text));
            }
            catch (Exception ex)
            {
