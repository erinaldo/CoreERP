...
{
    [Serializable]
    public class NFeobsFisco : ParamsBase
    {
        ...
        public void Save()
        {
            try
            {
                ...
                sSql = @"SELECT IDOUTBOX FROM ZNFEOBSFISCO WHERE IDOUTBOX = ? AND NITEM = ?";

                if (_conn.ExecHasRows(sSql, this.IdOutbox, this.nItem))
                {
                    sSql = @"UPDATE ZNFEOBSFISCO SET XCAMPO = ?, XTEXTO = ? WHERE IDOUTBOX = ? AND NITEM = ?";
                    _conn.ExecQuery(sSql, this.xCampo, this.xTexto, this.IdOutbox, this.nItem);
                }
                else
                {
                    sSql = @"INSERT INTO ZNFEOBSFISCO (IDOUTBOX, NITEM, XCAMPO, XTEXTO)
                                VALUES (?, ?, ?, ?)";
            ...
            catch (Exception ex)
            {
                ...
                ValidateLib.Log.SalvarLog("NFeobsFisco.Save", err);
                throw new Exception(err);
            }
        }

        public static NFeobsFisco ReadNFeobsFisco(params object[] parameters)
        {
            NFeobsFisco _nfeobsFisco = new NFeobsFisco();

            try
            {
                string sSql = @"SELECT * FROM ZNFEOBSFISCO WHERE IDOUTBOX = ?";
                _nfeobsFisco.ReadFromCommand(sSql, parameters);
            }
            catch (Exception ex)
            {
