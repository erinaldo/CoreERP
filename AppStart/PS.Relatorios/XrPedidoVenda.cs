...
{
    public partial class XrPedidoVenda : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XrPedidoVenda(List<PS.Lib.DataField> Params)
        {
            InitializeComponent();
...
";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                xrLabel97.Text = dt.Rows[0]["EMAIL"].ToString();
...
GOPER.PERCDESCONTO, 
(GOPERITEM.VLTOTALITEM + GOPERITEM.VLDESCONTO) VLTOTALITEM, 
...
FROM GOPER INNER JOIN GOPERITEM ON GOPER.CODEMPRESA = GOPERITEM.CODEMPRESA AND GOPER.CODOPER = GOPERITEM.CODOPER 
...
WHERE 
 ...
 ";
            ...
            if (dt.Rows.Count > 0)
            {
                xrLabel99.Text = string.Format("{0:n2}", dt.Rows[0]["VALORBRUTO"]);
                xrLabel102.Text = string.Format("{0:n2}", dt.Rows[0]["VALORFRETE"]);
                xrLabel110.Text = string.Format("{0:n2}", dt.Rows[0]["VALORDESCONTO"]);
                xrLabel105.Text = string.Format("{0:n2}", dt.Rows[0]["VALORDESPESA"]);
                xrLabel107.Text = string.Format("{0:n2}", dt.Rows[0]["VALORSEGURO"]);
                xrLabel100.Text = string.IsNullOrEmpty(dt.Rows[0]["IPI"].ToString()) ? "0,00" : string.Format("{0:n2}", dt.Rows[0]["IPI"]);
                xrLabel109.Text = string.Format("{0:n2}", Convert.ToDecimal(xrLabel99.Text) + Convert.ToDecimal(xrLabel102.Text) + Convert.ToDecimal(xrLabel107.Text) + Convert.ToDecimal(xrLabel100.Text) + Convert.ToDecimal(xrLabel105.Text) - Convert.ToDecimal(xrLabel110.Text));
