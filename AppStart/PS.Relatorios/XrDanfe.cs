...
namespace Relatorios
{
    ...
    public partial class XrDanfe : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void Detail_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                // Bloco de código comentado pois a data quando convertida retornava o dia anterior. João Pedro Luchiari, 20/02/2018.
                ...
                if (dsXML.Tables["ide"].Columns["dhEmi"] != null)
                {
                    txtDataEmissao.Text = dsXML.Tables["ide"].Rows[0]["dhEmi"].ToString();

                    if (txtDataEmissao.Text.Contains("T"))
                    {
                        string NovaData = txtDataEmissao.Text.Replace("T", " ");

                        NovaData = NovaData.Remove(10, 15);

                        txtDataEmissao.Text = Convert.ToDateTime(NovaData.ToString()).ToShortDateString();
                    }
                }

                if (dsXML.Tables["ide"].Columns["dhSaiEnt"] != null)
                {
                    txtDataSaidaDest.Text = dsXML.Tables["ide"].Rows[0]["dhSaiEnt"].ToString();

                    if (txtDataSaidaDest.Text.Contains("T"))
                    {
                        string NovaData2 = txtDataSaidaDest.Text.Replace("T", " ");

                        NovaData2 = NovaData2.Remove(10, 15);

                        txtDataSaidaDest.Text = Convert.ToDateTime(NovaData2.ToString()).ToShortDateString();
                    }
                }
                if (dsXML.Tables["ide"].Columns["dhSaiEnt"] != null)
                {
            ...
            if (dsXML.Tables["infAdic"] != null)
            {
                ...
                if (dsXML.Tables["infAdic"].Columns["InfAdFisco"] != null)
                {
                    xrLabel85.Text = xrLabel85.Text + "\n\rInf. Fisco:" + dsXML.Tables["infAdic"].Rows[0]["infAdFisco"].ToString();
                }
            }
        }
        ...
        private void DetailReport1_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            xrLabel104.DataBindings.Add("Text", null, "OriON");
                    ...
                    case XmlNodeType.Element:
                        ...
                        switch (elemento)
                        {
                            ...
                            case "vProd":
                                ...
                                break;
                            case "infAdProd":
                                item.xProd = item.xProd + " " + reader.ReadString();
                                break;
                            case "NCM":
                            ...
                            case "CST":
                                if (icms.Equals(true))
                                {
                                    ...
                                    item.OriON = item.orig.ToString() + item.CST;

                                    icms = false;
                                }
                                if (csosn == true)
                                {
                                    item.OriON = item.orig.ToString() + item.CSOSN.ToString();
                                    item.CST = reader.ReadString();
                                    icms = false;
                                }
                                //
        ...
        private void PageHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (dsXML.Tables["emit"].Columns["xFone"] != null)
            {
                txtNomeEmitente.Text = txtNomeEmitente.Text + "\n" + dsXML.Tables["emit"].Rows[0]["xFone"].ToString();
            }
        ...
        public void MarcaDagua()
        {
            string Status = dbs.QueryValue(string.Empty, "SELECT CODSTATUS FROM GNFESTADUAL WHERE CODEMPRESA = ? AND CODOPER = ?", this.Parametros[0].Valor, this.Parametros[1].Valor).ToString();

            if (Status == "C")
            {
                Watermark.Text = "CANCELADA";
                Watermark.TextDirection = DevExpress.XtraPrinting.Drawing.DirectionMode.ForwardDiagonal;
                Watermark.Font = new Font(Watermark.Font.FontFamily, 100);
                Watermark.ForeColor = Color.Red;
                Watermark.TextTransparency = 150;
                Watermark.ShowBehind = false;
                Watermark.PageRange = "1,3-5";
            }
        }
