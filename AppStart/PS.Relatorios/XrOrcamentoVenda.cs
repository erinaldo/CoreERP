...
{
    public partial class XrOrcamentoVenda : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public XrOrcamentoVenda(List<PS.Lib.DataField> Params)
        {
            InitializeComponent();
...
";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                xrLabel91.Text = dt.Rows[0]["EMAIL"].ToString();
...
WHERE 
	...
	AND GOPER.CODOPER = ?";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    xrLabel62.Text = string.Format("{0:n2}", dt.Rows[0]["VALORDESCONTO"]);
                    xrLabel48.Text = string.Format("{0:n2}", dt.Rows[0]["VALORDESPESA"]);
                    xrLabel92.Text = string.Format("{0:n2}", dt.Rows[0]["VALORSEGURO"]);
                    xrLabel64.Text = string.Format("{0:n2}", (Convert.ToDecimal(xrLabel60.Text) + Convert.ToDecimal(xrLabel85.Text) + Convert.ToDecimal(xrLabel48.Text) + Convert.ToDecimal(xrLabel92.Text) - Convert.ToDecimal(xrLabel62.Text)));
