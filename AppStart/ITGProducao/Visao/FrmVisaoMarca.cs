...
{
    public partial class FrmVisaoMarca : Form
    {
        string tabela = "PMARCA";
        ...
        public FrmVisaoMarca(string _query, Form frmprin)
        {
            InitializeComponent();
        ...
        public FrmVisaoMarca(string _where)
        {
            InitializeComponent();
            query = _where;
            carregaGrid(query);
        ...
        public FrmVisaoMarca(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string codigo = row1[tabela + ".CODMARCA"].ToString();

                DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM PRECURSO WHERE CODEMPRESA = ? AND CODFILIAL = ? AND CODMARCA = ?", new object[] { AppLib.Context.Empresa, AppLib.Context.Filial, codigo });
                if (dt.Rows.Count > 0)
                {
                ...
                else
                {
                    if (MessageBox.Show("Deseja realmente excluir este registro?", "Informação do Sistema.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        try
                        {
                            conn.ExecTransaction("DELETE FROM PMARCA WHERE CODEMPRESA = ? AND CODFILIAL = ? AND CODMARCA = ?", new object[] { AppLib.Context.Empresa, AppLib.Context.Filial, codigo });
                            conn.Commit();
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                FrmMarca frm = new FrmMarca(ref this.lookup);
                frm.edita = false;
                frm.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    FrmMarca frm = new FrmMarca();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    frm.codMarca = row1[tabela + ".CODMARCA"].ToString();
                    frm.edita = true;
                    frm.ShowDialog();
                    atualizaColuna(row1);
                }
            ...
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                //lookup.txtcodigo.Text = row1[tabela + ".CODMARCA"].ToString();
                lookup.txtconteudo.Text = row1[tabela + ".DESCMARCA"].ToString().ToUpper();

                switch (lookup.CampoCodigo_Igual_a)
                {
                    case NewLookup.CampoCodigoIguala.CampoCodigoBD:
                        lookup.txtcodigo.Text = row1[tabela + ".CODMARCA"].ToString();
                        lookup.ValorCodigoInterno = row1[tabela + ".CODMARCA"].ToString();
                        break;
