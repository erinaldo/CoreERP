...
{
    public partial class FrmVisaoOrdemMotivoParada : Form
    {
        string tabela = "PORDEMMOTIVOPARADA";
        ...
        public FrmVisaoOrdemMotivoParada(string _query, Form frmprin)
        {
            InitializeComponent();
        ...
        public FrmVisaoOrdemMotivoParada(string _where)
        {
            InitializeComponent();
            query = _where;
            carregaGrid(query);
        ...
        public FrmVisaoOrdemMotivoParada(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string codigo = row1[tabela + ".CODMOTIVOPARADA"].ToString();

                DataTable dt = AppLib.Context.poolConnection.Get("Start").ExecQuery("SELECT * FROM PORDEMAPONTAMENTO WHERE CODEMPRESA = ? AND CODFILIAL = ? AND CODMOTIVOPARADA = ?", new object[] { AppLib.Context.Empresa, AppLib.Context.Filial, codigo });
                if (dt.Rows.Count > 0)
                {
                    MessageBox.Show("Esta registro esta vinculado a um apontamento e não pode ser excluído.", "Informação do Sistema.", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    if (MessageBox.Show("Deseja realmente excluir este registro?", "Informação do Sistema.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        try
                        {
                            conn.ExecTransaction("DELETE FROM PORDEMMOTIVOPARADA WHERE CODEMPRESA = ? AND CODFILIAL = ? AND CODMOTIVOPARADA = ?", new object[] { AppLib.Context.Empresa, AppLib.Context.Filial, codigo });
                            conn.Commit();
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            if (this.lookup == null)
            {
                FrmOrdemMotivoParada frm = new FrmOrdemMotivoParada();
                frm.edita = false;
                frm.ShowDialog();
                carregaGrid(query);
            }
            ...
            {
                FrmOrdemMotivoParada frm = new FrmOrdemMotivoParada(ref this.lookup);
                frm.edita = false;
                frm.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                if (gridView1.SelectedRowsCount > 0)
                {
                    FrmOrdemMotivoParada frm = new FrmOrdemMotivoParada();
                    DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                    frm.codMotivo = row1[tabela + ".CODMOTIVOPARADA"].ToString();
                    frm.edita = true;
                    frm.ShowDialog();
                    atualizaColuna(row1);
                }
            ...
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                lookup.txtcodigo.Text = row1[tabela + ".CODMOTIVOPARADA"].ToString();
                lookup.txtconteudo.Text = row1[tabela + ".DESCMOTIVOPARADA"].ToString().ToUpper();
                lookup.ValorCodigoInterno =  row1[tabela + ".CODMOTIVOPARADA"].ToString();
                this.Dispose();
            }
        }
