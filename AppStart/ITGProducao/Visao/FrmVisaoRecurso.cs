...
{
    public partial class FrmVisaoRecurso : Form
    {
        ...
        public FrmVisaoRecurso(string _query, Form frmprin)
        {
            InitializeComponent();
        ...
        public FrmVisaoRecurso(string _where)
        {
            InitializeComponent();
            query = _where;
            carregaGrid(query);
        ...
        public FrmVisaoRecurso(ref NewLookup lookup)
        {
            InitializeComponent();
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string codigo = row1[tabela + ".CODRECURSO"].ToString();
                ...
                if (validaExcluir(ref vinculo, codigo) == true)
                {
                    if (MessageBox.Show("Deseja realmente excluir este registro?", "Informação do Sistema.", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        try
                        {
                            conn.ExecTransaction("DELETE FROM PRECURSO WHERE CODEMPRESA = ? AND CODFILIAL = ? AND CODRECURSO = ?", new object[] { AppLib.Context.Empresa, AppLib.Context.Filial, codigo });
        ...
        private void btnNovo_Click(object sender, EventArgs e)
        {
            ...
            {
                FrmRecurso frm = new FrmRecurso(ref this.lookup);
                frm.edita = false;
                frm.ShowDialog();
                this.Dispose();
            }
        ...
        private void Atualizar()
        {
            if (this.lookup == null)
            {
                ...
                FrmRecurso frm = new FrmRecurso();
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                frm.codRecurso = row1[tabela + ".CODRECURSO"].ToString();
                frm.edita = true;
                frm.ShowDialog();
                atualizaColuna(row1);
                }
            ...
            {
                DataRow row1 = gridView1.GetDataRow(Convert.ToInt32(gridView1.GetSelectedRows().GetValue(0).ToString()));
                lookup.txtconteudo.Text = row1[tabela + ".DESCRECURSO"].ToString().ToUpper();

                switch (lookup.CampoCodigo_Igual_a)
                {
                    case NewLookup.CampoCodigoIguala.CampoCodigoBD:
                        lookup.txtcodigo.Text = row1[tabela + ".CODRECURSO"].ToString();
                        lookup.ValorCodigoInterno = row1[tabela + ".CODRECURSO"].ToString();
                        break;
